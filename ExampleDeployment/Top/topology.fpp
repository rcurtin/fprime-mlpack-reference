module ExampleDeployment {
  ...
  deployment topology ExampleDeployment {
  ...
  # ----------------------------------------------------------------------
    ...
    telemetry connections instance CdhCore.tlmSend {
        # We must specify our telemetry connections manually, so that they do
        # not get automatically connected to CdhCore.tlmSend.TlmRecv (we need
        # to instead connect them to tlmSplitter, below).
        rateGroup_1Hz
        rateGroup_0_5Hz
        rateGroup_0_25Hz
        cmdSeq
  ...
  # ----------------------------------------------------------------------
    ...
    # include "ExampleDeploymentPackets.fppi"
  ...
  # ----------------------------------------------------------------------
    ...
    connections RateGroups {
      ...
      rateGroup_1Hz.RateGroupMemberOut[6] -> anomalyDetector.run
    ...
    connections ExampleDeployment {
      systemResources.Tlm -> tlmSplitter.TlmRecv
      # In a deployment with more sensors, each of those components should also
      # be connected to tlmSplitter.TlmRecv.
      tlmSplitter.TlmOutA -> CdhCore.tlmSend.TlmRecv
      tlmSplitter.TlmOutB -> anomalyDetector.tlmIn
    }
